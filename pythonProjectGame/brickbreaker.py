...
floor = pygame.Rect(100, 550, 150, 10)  #
...
while continueGame:
    ...
    if floor.collidepoint(ball.x, ball.y):
        ...
        # Calculate the angle of reflection based on where the ball hits the bar
        angle = (ball.x + ball.width / 2 - floor.x - floor.width / 2) / (floor.width / 2)
        angle = math.radians(angle * 60)
        move[0] = math.sin(angle) * 5
        move[1] = -math.cos(angle) * 5
